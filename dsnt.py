...
def dsnt(inputs):
    ...
    outputs_x = tf.reduce_sum(tf.multiply(inputs, dsnt_x), axis=[1, 2])
    outputs_y = tf.reduce_sum(tf.multiply(inputs, dsnt_y), axis=[1, 2])

    # Zip into [x, y] pairs
    outputs_zipped = tf.stack([outputs_x, outputs_y], axis=1)

    return outputs_zipped
...
def js_reg_loss(heatmaps, centres):
    '''
...
def _make_gaussians(centres_in, height, width):
    '''
    ...
    def body(centres, heatmaps):
        curr = centres[0]
        centres = centres[1:]
        new_heatmap = _make_gaussian([height, width], curr)
